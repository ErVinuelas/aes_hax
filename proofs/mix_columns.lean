...
def mix_columns_state_spec (st : Vector u16 8) : Vector u16 8 :=
  ...
  let to_lsb (a : Nat) := 15 - a
  let s_0 := get_elem_bv st (to_lsb index)
  let s_1 := get_elem_bv st (to_lsb (index + 1))
  let s_2 := get_elem_bv st (to_lsb (index + 2))
  let s_3 := get_elem_bv st (to_lsb (index + 3))
  ...
  let set_0 := set_elem_bv res (to_lsb index) s_0'
  let set_1 := set_elem_bv set_0 (to_lsb (index + 1)) s_1'
  let set_2 := set_elem_bv set_1 (to_lsb (index + 2)) s_2'
  set_elem_bv set_2 (to_lsb (index + 3)) s_3'
